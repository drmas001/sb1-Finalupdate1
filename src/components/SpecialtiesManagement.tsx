...
const SpecialtiesManagement: React.FC = () => {
  ...
  const generateRowKey = (patient: Patient | Consultation) => {
    const timestamp = 'admission_date' in patient ? patient.admission_date : patient.created_at;
    return `${patient.mrn}-${timestamp}-${Math.random().toString(36).substr(2, 9)}`;
  };
                    ...
                    <tr key={generateRowKey(patient)}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {'specialty' in patient ? 'Admission' : 'Consultation'}
                        </div>
                      </td>
